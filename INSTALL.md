...
Below v22.19, or `command not found`: install Node 22 LTS or newer from https://nodejs.org (or the user's version manager). The launcher finds the CLI through `npx`, so nothing else from npm has to be installed. Optional, to skip the npx download on every run: `npm install -g @liustack/flipbook@0.5.6`.
