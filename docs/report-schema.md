...
| `audio-silent` | render | The timeline asks for music but the soundtrack measures as silence. With `audio.mode: "file"`, `detail` gives `file`, `offsetSec` and `fileDurationSec`: most often the offset starts past the end of the file | Lower `audio.offset` below the file length, or pick a file that has sound where the film uses it |
