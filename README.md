...
```text
Install flipbook following https://github.com/liustack/flipbook/blob/v0.5.5/INSTALL.md, render the hello example, and tell me the result.
```
...
You need Node 22.19 or newer and ffmpeg with libx264 (`brew install ffmpeg` on macOS, `sudo apt-get install -y ffmpeg` on Debian and Ubuntu). The first check or render downloads a pinned Chromium (about 95 MB) and two Chinese fonts (about 50 MB) into your user cache. To skip the npx download on every run, install the skill's renderer globally: `npm install -g @liustack/flipbook@0.5.5`.
