...
- **多页并行**：check 通过后，render 按 CPU 核数减一开浏览器，最多 6 个，每个一页，谁空下来谁接下一帧，帧按顺序送进同一个 ffmpeg，逐帧哈希和单页相同。页数再按内存和片长封顶，`--jobs` 可改。合成最近一次 check 没退 0、或之后改过文件，就只用一页，`render.parallel.reason` 写明原因。
