...
- **`examples/riffle/`**：一本摊在桌上的手翻小本。Rijksmuseum 藏的迈布里奇奔马连拍（CC0，经 `stock fetch` 下载）由 `specimens()` 自动切成 20 格，按阅读顺序印在书页上。先慢慢翻两页，再越翻越快到每秒十页，马就跑起来了，最后慢下来停在站着的马上，出片名。书页是用旧了的廉价纸：发黄、锈斑、拇指蹭脏的角、磨圆的边，手裁的纸页参差不齐。镜头和本子按一拍二走，书页每帧都翻，所以翻页照样流畅。一共 12.5 秒。
