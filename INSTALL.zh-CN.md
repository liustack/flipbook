...
低于 v22.19 或者 `command not found`：从 https://nodejs.org（或用户的版本管理器）装 Node 22 LTS 或更新的版本。启动器经 `npx` 找 CLI，npm 上别的东西都不用装。可选，想省掉每次 npx 的下载：`npm install -g @liustack/flipbook@0.5.0`。
