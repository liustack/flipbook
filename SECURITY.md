...
- flipbook 只往合成目录的 `.flipbook/` 和 `out/` 里写。写之前从合成目录的真实路径逐级核对，这两处或其中任何一级是软链就整条命令拒写（`unsafe-output`），文件先写新名字再改名替换，不会顺着软链或硬链写到外面。`audio.file` 解析软链后也必须是合成目录里的普通文件，ffmpeg 只按本地文件、白名单格式读它。
