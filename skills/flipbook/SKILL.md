...
If scripts cannot run, use the first line that works (the pinned version is 0.5.3):

1. A `flipbook` on PATH with the same major.minor as the pinned version and not older than it: `flipbook <args>`.
2. `npx --yes --package @liustack/flipbook@0.5.3 flipbook <args>`.
3. `bunx --bun @liustack/flipbook@0.5.3 <args>`.
