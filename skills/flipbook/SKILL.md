...
After a passing check, render draws on several pages at once (CPU cores minus one, at most 6, fewer for short films or big frames). `--jobs <n>` sets the count. `snapshot` takes `--size` too.
