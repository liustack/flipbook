...
| `fall(t, { at, x, y, seed, speed, sway, wind, spin, floor })` | `{ x, y, rotate, flip, falling }` for a petal, leaf or scrap of paper that lets go at `at`: it speeds up, drifts down swinging side to side, turns, and tumbles (scale its width by `flip`, near 0 it is edge-on). With `floor` it lands and lies flat. `wind` blows it sideways, a strong wind and a low `speed` carry it across the frame |
...
- Use only flipbook's fonts, `"Noto Serif SC"` (`FONTS.serif`, weights 200 to 900) and `"LXGW WenKai"` (`FONTS.hand`, weight 400), or fonts the user supplied with a license in brand.json or `assets/fonts/` (see `brand.md`). Both flipbook fonts cover Chinese and Latin. Other font names fall back to system fonts and fail with `font-fallback`. Characters none of these fonts has fail with `missing-glyph`. Canvas text is checked character by character against the families in `ctx.font`, in order: a character the named font lacks fails with `font-fallback` even when the other flipbook font has it, so list both (`"Noto Serif SC", "LXGW WenKai"`) when in doubt.
