...
- Never change a DOM text element's `transform: scale()` from frame to frame. Chromium caches glyphs by size, so two separate renders of the same frame come out different, and check cannot see it because it compares frames inside one page. Text that grows or shrinks goes on a canvas: `ctx.scale()` or a changing font size, then `fillText()`. Moving DOM text with `translate` is fine.
